import csv
sql="INSERT INTO funeraria_contrato_individual VALUES"

def acomodarFechas(fechaSinFormatear):
    if "/" in fechaSinFormatear:
        intermedio=fechaSinFormatear.split("/")
        fechaFormateada=intermedio[2]+"-"+intermedio[1]+"-"+intermedio[0]
        return fechaFormateada
    else:
        return fechaSinFormatear

with open('plantillaContratos.csv') as csv_file:
    file = open('ContratosSqlTratado.sqlZip','w')
    csv_reader = csv.reader(csv_file, delimiter=',')
    for contador,parametros in enumerate(csv_reader):
        if contador > 10:
            parametros[7]=acomodarFechas(parametros[7])
            parametros[54]=acomodarFechas(parametros[54])
            parametros[55]=acomodarFechas(parametros[55])
            listaDeParametros={
